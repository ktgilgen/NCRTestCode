...
function LineGraph(argsMap) {
	 ...
	 */
	this.slideData = function(newData,mod) { //this.slideData = function(newData,mod) {
		// validate data
		...
		data.startTime = new Date(data.startTime.getTime() + (data.step * numSteps + mod));
		data.endTime = tempData.endTime;
		debug("Updated startTime: " + data.startTime + "  endTime: " + data.endTime);
		/*
		var t1 = document.getElementById('para1').innerHTML= data.startTime;
		var t2 = document.getElementById('para2').innerHTML= data.endTime;
		var t3 = document.getElementById('para3').innerHTML= data.startTime-data.endTime;
			*/	
		/*
	...
	var createGraph = function() {
		...
		linesGroup.append("path")
				.attr("class", function(d, i) {
					...
					return "line series_" + getOptionalVar(argsMap,'id',0) + getOptionalVar(argsMap,'cells',[])[i];
				})
				.attr("fill", "none")
